...
function guess_shutter_frames(
    ...
    exposed_thresh, dark_thresh = two_gaussian_thresholds(image_intensities, thresh_stds)
...
function align_feather_files(
    ...
    isnothing(m) && throw(ArgumentError("Invalid video file $vidfile"))
    ...
    if isnothing(exposed_thresh) || isnothing(dark_thresh)
        exposed_thresh, dark_thresh = two_gaussian_thresholds(intensities, thresh_stds)
    end
...
align_feather_files(dir::AbstractString) = align_feather_files(joinpath.(dir, readdir(dir)))
